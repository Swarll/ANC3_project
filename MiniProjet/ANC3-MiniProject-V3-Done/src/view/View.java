...
public class View extends VBox {
    ...
    private final IntegerProperty numLineToSelect = new SimpleIntegerProperty();
    ...
    {
        toDoList.itemsProperty().bind(vm.ToDoListProperty());
        doneList.itemsProperty().bind(vm.DoneListProperty());
        addText.textProperty().bindBidirectional(vm.addTextProperty());
    }
    
    private void configActionsBindings()
    {
        setDone.disableProperty().bind(vm.setDoneProperty());
        setToDo.disableProperty().bind(vm.setToDoProperty());
        addButton.disableProperty().bind(vm.addButtonProperty());
    }
    
    private void configViewModelBindings()
    {
        vm.indexToDoProperty().bind(toDoListProperty().selectedIndexProperty());
        vm.indexDoneProperty().bind(doneListProperty().selectedIndexProperty());
    }
    
    private SelectionModel<String> toDoListProperty(){
        return toDoList.getSelectionModel();
    }
    
    private SelectionModel<String> doneListProperty(){
    ...
    {
        toDoList.getItems().setAll(vm.ToDoListProperty());
        doneList.getItems().setAll(vm.DoneListProperty());
    }
    ...
    private void configButton2()
    {
        setDone.setOnAction(e -> {
            vm.transfer(toDoList, doneList, true);
        });

        setToDo.setOnAction(e -> {
            vm.transfer(doneList, toDoList, false);
        });

        addButton.setOnAction(e -> {
            if ((vm.addToDo(addText.getText())) == true)
                addText.setText("");
        });
    }
