...
public class DirectoryComparator {
    ...
    private void getFileState(SimpleFile f, SimpleFile tmp)
    {
        ...
        if ((f.getParentDirectory() != null && tmp.getParentDirectory() != null && f.getParentDirectory().getName().compareTo(tmp.getParentDirectory().getName()) == 0) ||
                (f.getParentDirectory() != null && tmp.getParentDirectory() != null && f.getParentDirectory().getParentDirectory() == null && tmp.getParentDirectory().getParentDirectory() == null))
        {
    ...
    private void checkState(Directory toCompare, int state[])
    {
        ...
        {
            if (toCompare.getNChild(i).isDirectory())
            {
                checkState((Directory)toCompare.getNChild(i), state);
                increment(toCompare.getNChild(i), state);
            }
            else
            {
                increment(toCompare.getNChild(i), state);
    ...
    private void compareDirectory(Directory toCompare, Directory compareWith)
    {
        ...
        {
            if (compareWith.getNChild(i).isDirectory())
            {
                compareDirectory(toCompare, (Directory)compareWith.getNChild(i));
            }
        }
    ...
    private void compareWith3(Directory toCompare, Directory compareWith)
    {
        ...
        {
            if (toCompare.getNChild(i).isDirectory())
                compareWith3((Directory)toCompare.getNChild(i), compareWith);
        }
    ...
    private void compareFile(SimpleFile f, Directory c)
    {
        ...
        {
            if (c.getNChild(i).isDirectory())
                compareFile(f, (Directory)c.getNChild(i));
            else
                getFileState(f, (SimpleFile)c.getNChild(i));
        }
    }
    ...
    private void compareWith2(Directory d, Directory c)
    {
        ...
        {
            if (d.getNChild(i).isDirectory())
                compareWith2((Directory)d.getNChild(i), c);
            else
                compareFile((SimpleFile) d.getNChild(i), c);
        }
    }
    ...
    public void compare(Directory d1, Directory d2)
    {
        ...
        {
            if (d1.getNChild(i).isDirectory())
                compareWith2((Directory)d1.getNChild(i), d2);
            else
                compareFile((SimpleFile) d1.getNChild(i), d2);
        }
        for (int j = 0; j < d1.getChild().size(); ++j)
        {
            if (d1.getNChild(j).isDirectory())
            {
                compareWith3((Directory)d1.getNChild(j), d2);
                compareDirectory((Directory)d1.getNChild(j), d2);
            }
        }
    ...
    public void rebootCorresp(FileOrDirectory fod)
    {
        if (fod != null && fod instanceof Directory)
        {
            ...
            {
                rebootCorresp(((Directory)fod).getNChild(i));
            }
        }
